...
stock Player::InsertData(playerid, GLOBAL_TAG_TYPES:...)
{
    ...
    if(!DB::GetRowCount(db_entity, "players", "name", name, db_result))
    {
        ...
        printf("[ DB ] [ (ENTITY) PLAYER ] Conta do jogador %s, adicionada ao banco\n", name);
        return 1;
    }
...
stock Player::LoadAllData(playerid)
{
    ...
    pdy::Player[playerid][pdy::time_left]   = DB_GetFieldIntByName(db_result, "payday_tleft"); 
...
//     if(DB_GetRowCount(db_entity, db_result))
//     {
//         DB_FreeResultSet(db_result);
